...
- ✅ 导航条目的目标 URL 存放于自定义字段 `_sites_link`，由模板直接输出、不走 `the_content` 过滤器
- ✅ 采用 **output buffer** 方案：仅替换最终 HTML 中指向外链的 `<a href="...">`，**不修改 `_sites_link` 字段本身**
- ✅ 因此主题原有的 favicon 接口抓取、二维码拼接、缩略图判断均不受影响（图标正常显示）
- ✅ 前台所有页面(首页、分类、搜索、详情)统一接管外链，后台编辑、REST/AJAX 接口不受影响
