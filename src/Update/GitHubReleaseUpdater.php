...
final class ExternalLink_GitHubReleaseUpdater
{
    private const GITHUB_OWNER  = 'Jacky088';
    private const GITHUB_REPO   = 'External-Link';
    private const PLUGIN_NAME   = '外链跳转插件';
    private const PLUGIN_AUTHOR = '木木';
    private const PLUGIN_DESC   = '外链跳转插件是一个非常实用的WordPress插件，它可以对文章中的外链进行过滤，有效地防止追踪和提醒用户。';
    private const WP_TESTED     = '7.0';
    private const WP_REQUIRES   = '6.4';
    private const PHP_REQUIRES  = '7.4';

    private const API_URL      = 'https://api.github.com/repos/Jacky088/External-Link/releases/latest';
    private const RELEASES_URL = 'https://github.com/Jacky088/External-Link/releases';
    private const CACHE_KEY    = 'external_link_latest_release';
    ...
    private static function getRelease()
    {
        ...
        if ($status < 200 || $status >= 300 || $body === '') {
            // GitHub API 限流（匿名 60 次/小时）时静默降级，短时间缓存避免反复请求报错
